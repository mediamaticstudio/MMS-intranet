...
const regions = [
  ...
  { name: 'UNITED STATES', percentage: 40, icon: '🇺🇸', color: 'bg-[#FDE68B]' },
  { name: 'UNITED KINGDOM', percentage: 25, icon: '🇬🇧', color: 'bg-[#FDE68B]' },
  { name: 'AUSTRALIA', percentage: 20, icon: '🇦🇺', color: 'bg-[#FDE68B]' },
  { name: 'REST OF WORLD', percentage: 15, icon: '🌍', color: 'bg-[#FDE68B]/20' },
]

export default function GeographicStrategy() {
  return (
    <section id="global-strategy" className="bg-[#181818] py-32 px-6 md:px-20 border-t border-[#FDE68B]/5 relative overflow-hidden">

      {/* Background Graphic - Burgundy Vision */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-full text-[#FDE68B]/5 flex items-center justify-center select-none pointer-events-none">
      ...
      <div className="max-w-7xl mx-auto relative z-10 flex flex-col md:flex-row items-center justify-between gap-20">
              ...
              >
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-4">
                    ...
                    <span className="text-lg font-bold text-[#FDE68B] tracking-tight group-hover:opacity-70 transition-all uppercase">
                      {region.name}
                    </span>
                  </div>
                  <span className="text-2xl font-black text-[#FDE68B]">{region.percentage}%</span>
                </div>

                {/* Progress Bar Container - Burgundy scheme */}
                <div className="w-full h-2 rounded-full bg-[#FDE68B]/5 border border-[#FDE68B]/5 overflow-hidden">
                  <motion.div
                    initial={{ width: 0 }}
                    whileInView={{ width: `${region.percentage}%` }}
                    transition={{ duration: 1.5, delay: 0.5, ease: "circOut" }}
                    viewport={{ once: true }}
                    className={`h-full bg-[#FDE68B] rounded-full relative`}
                  >
        ...
        <div className="w-full md:w-1/3 flex flex-col items-center justify-center p-12 bg-[#FDE68B] rounded-full border-2 border-[#FDE68B]/30 relative animate-[pulse_5s_infinite] shadow-2xl shadow-[#FDE68B]/40">
          ...
          {/* Pulsing ring */}
          <div className="absolute inset-0 rounded-full border border-[#FDE68B]/20 scale-125 animate-ping opacity-20" />
        </div>
